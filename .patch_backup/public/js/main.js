...
async function showInAppNotification(serverName, roomName, senderName, text, serverId, serverData, roomId, isDm = false, targetUid = null, targetAvatarUrl = null) {
  ...
  // 通知音の再生（showNotification と共有の lastNotificationTime でデバウンスし2重再生を防ぐ）
  if (soundEnabled && notifEnabled) {
    const now = Date.now();
    const sameContent = displayBody === lastNotificationBody && (roomId || '') === lastNotificationRoomId;
    const isDuplicate = now - lastNotificationTime < 3000 && sameContent;
    if (!isDuplicate) {
      // アプリがフォーカス中（＝OS通知が送られない場合）はここで時刻を更新する
      // バックグラウンド時は showNotification 側でも更新されるが上書きしても問題ない
...
async function showNotification(title, body, roomId) {
  ...
  // アプリがフォーカスされてオンラインの場合はOS通知不要（アプリ内トーストで十分）
  ...
  // 通知音は showInAppNotification 側で一元管理するためここでは再生しない
