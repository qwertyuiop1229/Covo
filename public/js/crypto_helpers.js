...
export const E2EE_PREFIX = "enc::v";       // 暗号文の目印（過去の平文と区別する）
    ...
    async function _ensureE2EEKeys() {
    ...
    async function __ensureE2EEKeysImpl() {
      try {
        // 1) ローカル(JWK)から復元
    ...
    async function __backupKeysToFirestore(privJwk, pubJwk) {
    ...
    async function __getUserPublicKey(uid) {
    ...
    async function __getEscrowPublicKey() {
    ...
    async function _requestEscrowRescue(serverId, roomId) {
      if (!_getUserId()) return;
      const now = Date.now();
    ...
    async function _ensureEscrowKey() {
    ...
    async function _getOrCreateRoomKey(serverId, roomId, memberIds) {
    ...
    async function __getOrCreateRoomKeyImpl(serverId, roomId, memberIds) {
      if (!_subtleOK) return null;
    ...
    async function _getRoomKeyWithWait(serverId, roomId, memberIds, maxWaitMs = 1500) {
      if (!_subtleOK) return null;
      if (_e2ee.roomKeyCache[roomId]) return _e2ee.roomKeyCache[roomId];
    ...
    async function _rotateAllRoomKeys(serverId, remainingMembers) {
       if (!_subtleOK) return;
       try {
    ...
    async function __distributeRoomKeyVersion(serverId, roomId, rawKey, memberIds, version) {
    ...
    async function _backfillRoomKeysForMembers(serverId, roomId, memberIds) {
    ...
    async function _encryptText(plaintext, roomKeyObj) {
    ...
    function _isEncrypted(text) {
      return typeof text === "string" && text.startsWith(E2EE_PREFIX);
    }

    async function _decryptText(text, serverId, roomId, memberIds) {
    ...
    async function _decryptMessagesInPlace(messages, serverId, roomId, memberIds) {
    ...
    // --- ファイルのE2EE暗号化/復号化 ---
    async function _encryptFileE2EE(file, roomKeyObj) {
    ...
    async function _decryptFileE2EE(encryptedBuffer, roomKeyObj, serverId = null, roomId = null) {
    ...
    async function _updateE2EEStatusUI() {
