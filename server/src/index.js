...
function isAllowedOrigin(origin) {
  ...
  if (
    ...
    origin === "https://covo.app" ||
    origin === "https://simplechat-65a0d.web.app" ||
    origin === "https://simplechat-65a0d.firebaseapp.com"
  ) {
    return true;
  }
  // ローカル開発環境（ポート番号付き: http://localhost:5173, http://127.0.0.1:8080 等）
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/agoraToken" && (request.method === "GET" || request.method === "POST")) {
        return await handleAgoraToken(request, env, url);
      }
...
async function handleJoinServer(request, env) {
  ...
  try {
    ...
    if (password) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        if (invData.fields.disabled && invData.fields.disabled.booleanValue) {
          return new Response(JSON.stringify({ success: false, error: "Invite code is disabled" }), { status: 403, headers: { ...cors, "Content-Type": "application/json" } });
        }
        
        let expiresAt = invData.fields.expiresAt?.timestampValue;
        if (expiresAt && new Date(expiresAt).getTime() < Date.now()) {
          return new Response(JSON.stringify({ success: false, error: "Invite code expired" }), { status: 403, headers: { ...cors, "Content-Type": "application/json" } });
        }
    ...
    if (!valid) {
      return new Response(JSON.stringify({ success: false, error: "Authentication required" }), { status: 400, headers: { ...cors, "Content-Type": "application/json" } });
    }
    ...
    if (updateData.error) {
       ...
       return new Response(JSON.stringify({ success: false, error: "Failed to join server" }), { status: 500, headers: { ...cors, "Content-Type": "application/json" } });
    }
...
// -------------------------------------------------------------
// Agora RTC Token (v006) 動的生成処理
// -------------------------------------------------------------
const AGORA_CRC_TABLE = new Uint32Array(256);
for (let i = 0; i < 256; i++) {
  let c = i;
  for (let j = 0; j < 8; j++) c = (c & 1) ? (0xEDB88320 ^ (c >>> 1)) : (c >>> 1);
  AGORA_CRC_TABLE[i] = c >>> 0;
}

function agoraCrc32(str) {
  const bytes = new TextEncoder().encode(str);
  let crc = 0xFFFFFFFF;
  for (let i = 0; i < bytes.length; i++) crc = (crc >>> 8) ^ AGORA_CRC_TABLE[(crc ^ bytes[i]) & 0xFF];
  return ((crc ^ 0xFFFFFFFF) >>> 0);
}

async function handleAgoraToken(request, env, url) {
  const cors = getCorsHeaders(request);
  try {
    ...
    if (!appId || !appCert) {
      return new Response(JSON.stringify({ success: false, error: "Agora credentials not configured on worker", appId: appId || "" }), {
        status: 200,
        headers: { ...cors, "Content-Type": "application/json" }
      });
    }
    ...
    for (const p of privileges) {
      ...
      msgView.setUint32(offset, p.val, true); offset += 4;
    }
    const rawMsg = new Uint8Array(msgBuf);

    const enc = new TextEncoder();
    const parts = [enc.encode(appId), enc.encode(channelName), enc.encode(uidStr), rawMsg];
    const totalLen = parts.reduce((acc, c) => acc + c.length, 0);
    const signPayload = new Uint8Array(totalLen);
    let signOff = 0;
    for (const p of parts) { signPayload.set(p, signOff); signOff += p.length; }

    const key = await crypto.subtle.importKey("raw", enc.encode(appCert), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const signature = new Uint8Array(await crypto.subtle.sign("HMAC", key, signPayload));

    const contentBuf = new ArrayBuffer(2 + signature.length + 4 + 4 + 2 + rawMsg.length);
    const contentView = new DataView(contentBuf);
    offset = 0;
    contentView.setUint16(offset, signature.length, true); offset += 2;
    new Uint8Array(contentBuf, offset, signature.length).set(signature); offset += signature.length;
    contentView.setUint32(offset, agoraCrc32(channelName), true); offset += 4;
    contentView.setUint32(offset, agoraCrc32(uidStr), true); offset += 4;
    contentView.setUint16(offset, rawMsg.length, true); offset += 2;
    new Uint8Array(contentBuf, offset, rawMsg.length).set(rawMsg); offset += rawMsg.length;

    let bin = "";
    const bArr = new Uint8Array(contentBuf);
    for (let i = 0; i < bArr.length; i++) bin += String.fromCharCode(bArr[i]);
    const token = "006" + appId + btoa(bin);

    return new Response(JSON.stringify({ success: true, token, appId }), {
      status: 200,
      headers: { ...cors, "Content-Type": "application/json" }
    });
  } catch (err) {
...
async function handleDownloadProxy(request, env, url) {
  const fileCors = getFileCorsHeaders(request);
  try {
    ...
    if (!targetUrlStr) {
      return new Response(JSON.stringify({ error: 'Missing URL parameter' }), {
        status: 400,
        headers: { ...fileCors, 'Content-Type': 'application/json' }
      });
    }
    ...
    const newHeaders = new Headers(fileCors);
...
async function handleServeFile(request, env, url) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: err.toString() }), {
      status: 500, headers: { ...fileCors, 'Content-Type': 'application/json' }
    });
  }
}
  ...
  try {
    ...
    if (subpath.startsWith("escrow/rescue")) {
      if (request.method === "POST") {
        ...
        // -------------------------------------------------------------
        // サーバーメンバー判定ヘルパー (プルーニング認可用)
        // -------------------------------------------------------------
        async function isServerMemberCheck(appId, serverId, verifiedUser, env) {
        if (!appId || !serverId || !verifiedUser) return false;
        if (env.DB) {
        try {
        const row = await env.DB.prepare("SELECT 1 FROM server_joined_users WHERE server_id = ? AND user_id = ? AND app_id = ?").bind(serverId, verifiedUser.uid, appId).first();
        if (row) return true;
        } catch (e) {
        console.error("isServerMemberCheck D1 error:", e);
        }
        }
        try {
        const adminToken = await getAdminTokenForFirestore(env);
        ...
        if (!srvData.error && srvData.fields) {
        if (srvData.fields.createdBy?.stringValue === verifiedUser.uid) return true;
        if (srvData.fields.joinedUsers?.arrayValue?.values) {
          const members = srvData.fields.joinedUsers.arrayValue.values.map(v => (v.stringValue || "").trim());
          if (members.includes(verifiedUser.uid.trim())) return true;
        }
        }
        }
        } catch (e) {
        console.error("isServerMemberCheck Firestore error:", e);
        }
        return false;
        }
